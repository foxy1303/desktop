...
var initialization = function() {
  ...
  var datasource = app.dataSources.sbt_datasource;  
  ...
  datasource.discoverModelDefinitions({schema: 'db', views: false, all: true}, function(error, models) {
      if (error) console.log(error); else {
        var promises = [];
        for (var index in models) {
          promises.push(new Promise((resolve, reject) => datasource.discoverAndBuildModels(models[index].name, {associations: true, nameMapper: (type, name) => name}, function(innerError, definition) {
              if (innerError) reject(innerError); else {
        ...
        modelsFile.then(content => Promise.all(promises).then(values => {
          values.forEach(modelName => content[modelName] = { public: true, dataSource: "sbt_datasource" });
